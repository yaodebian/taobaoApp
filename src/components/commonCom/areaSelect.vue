<template>
  <!--居住地址三级联动选项(网上网友的一个分享)-->
  <section
    v-show="showChose"
  >
    <div class="modal" @click="$parent.areaBox = false"></div>
    <div class="showChose">
      <section class="address">
        ...
        <ul>
          ...
          <li
            class="addList"
            v-for="(v, k, index) in showCityList"
            :key="index"
          ...
          <li
            class="addList"
            v-for="(v, k, index) in showDistrictList"
            :key="index"
...
export default {
  ...
  computed: {
    areaStr () {
      return this.City ? `${this.Province?this.Province:this.info[this.province-1].name} ${this.City?this.City:''} ${this.District?this.District:''}` : ''
    }
  },
  watch: {
    areaStr (newVal) {
      this.$store.dispatch('editArea', newVal)
    }
  },
  data() {
    return {
