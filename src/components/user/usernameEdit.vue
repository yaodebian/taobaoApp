<template>
  <div class="userEdit">
    <mt-field
      placeholder="请输入你的会员名(1-10位字符或中文)"
      v-model="username"
      class="userEdit-input"
    ></mt-field>
    <mt-field
      placeholder="请输入你的密码(6-12位，必须包含大小写字母和数字)"
      v-model="password"
      type="password"
      class="userEdit-input"
    ></mt-field>
...
export default {
  ...
  methods: {
    saveUsername() {
      ...
      } else if (username !== '' && usernameRegExp.test(username) && password !== '' && pswRegExp.test(password) && password !== checkPsw) {
        Toast({
          message: '两次输入密码不一致',
          position: "middle",
          duration: 2000
